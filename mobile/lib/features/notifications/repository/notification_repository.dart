...
class NotificationModel {
  ...
  const NotificationModel({
    ...
    this.caseId, required this.isRead, required this.createdAt,
  });

  factory NotificationModel.fromJson(Map<String, dynamic> j) => NotificationModel(
    id: j['id'] as String, type: j['type'] as String,
    title: j['title'] as String, body: j['body'] as String,
    caseId: (j['caseId'] as String?) ?? (j['caseEntry'] is Map<String, dynamic> ? (j['caseEntry']['id'] as String?) : null),
...
class NotificationRepository {
  ...
  Future<List<NotificationModel>> getNotifications({int page = 0}) async {
    final resp = await _dio.get('/api/notifications', queryParameters: {'page': page, 'size': 20});
